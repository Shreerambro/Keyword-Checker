...
try:
    from unrar import cffi as unrar_cffi  # unrar-cffi backend
    import rarfile
    rarfile.tool_setup(unrar_tool=None, force=True)  # disable CLI tool
    RAR_SUPPORT = True
    print("✅ RAR support enabled (cffi backend)")
except ImportError:
    try:
        import rarfile
        RAR_SUPPORT = True
        print("⚠️ RAR support enabled (CLI backend)")
    except ImportError:
        RAR_SUPPORT = False
        print("❌ RAR support disabled")
...
async def myid_command(client, message: Message):
    uid = message.from_user.id
    s = '✅ Owner' if is_owner(uid) else '✅ Admin' if is_admin(uid) else '❌ Not admin'
    ...
    if "search_terms" in fi and "output_filename" in fi:
        ...
        try:
            ...
            if of and os.path.exists(of):
                await message.reply_document(document=of, caption=f"✅ Done!\n🔍 Keywords: `{', '.join(fi['search_terms'])}`")
        ...
        try:
            ...
            if output_file and os.path.exists(output_file):
                ...
                if rs == 0:
                    ...
                    await message.reply_document(document=output_file, caption=f"✅ **Search Complete!**\n📝 Output: `{output_filename}`\n🔍 Keywords: `{', '.join(search_terms)}`\n📊 Size: {rs:,} bytes")
                    ...
                    if ri.filename.lower().endswith('.txt'):
                        ...
                        with rf.open(ri.filename) as f:
                            for lb in f:
                                ...
                                if lines - last_upd >= 500_000:
                                    last_upd = lines
                                    try: await status_msg.edit_text(f"🔍 **Searching...**\n📂 File {files_done}: `{ri.filename}`\n📊 Lines: {lines:,}\n✅ Matches: {found:,}")
                                    except: pass
        else:
            raise Exception(f"Unsupported format: {os.path.splitext(file_name)[1]}")
    except Exception as e:
        if os.path.exists(archive_path): os.remove(archive_path)
        es = str(e).lower()
    ...
    if cq.data == "archive_pwd_yes":
        ...
        try:
            ...
            if of and os.path.exists(of):
                ...
                await cq.message.reply_document(document=of, caption=f"✅ **Done!**\n🔍 Keywords: `{', '.join(fi['search_terms'])}`\n📄 Output: `{fi['output_filename']}`")
                os.remove(of); await status_msg.delete()
            else:
                await status_msg.edit_text("❌ No matches found!")
            user_states[uid] = {"state": "waiting_file"}
